...
In this contest, the goal is to detect (detect) and recognize (recognize) text in images, specifically here will focus on text in the scene (scene text) recorded from many different camera sources. in Vietnam, as well as collected from google images. In order to serve the development of HO CHI MINH City such as digitization, orientation for automatic robots, ...
### [Dataset](#dataset)
- [VinTextV2](#vintextv2)
